...
class TestComboModelDataset(unittest.TestCase):
    ...
    def test_different_tokenization_case(self):
        ...
        for idx, sentence in enumerate(train_dataset.sentences_pair_tokens):
            print(f"\nSentence {idx}: \n")
            for pair in sentence:
                input_token = pair[0].rstrip('_')
                target_token = pair[1].rstrip('_')
                self.assertTrue(input_token.startswith(target_token) or target_token.startswith(input_token), msg=f"In sentence {idx} pair: {pair} are not aligned with each other")
